...
from transformers.file_utils import WEIGHTS_NAME, is_apex_available, is_torch_tpu_available
from torch.utils.data.dataloader import DataLoader
# from transformers.optimization import get_scheduler
from compress_tools.optimization import get_scheduler
...
COMPONENT_NAME = ['FFN_1','FFN_2','attention']
...
class ChildTuningDtrainer(Trainer):
    def __init__(self, **kwargs):
        ...
        if _has_wandb:
            ...
            logger.info("wandb on.")
            self.callback_handler.add_callback(WandbCallback)
    ...
    def determine_weight_across_layers(self, gradient_name_mask, topN=5):
        choose_name = set()
        for target_name in ['intermediate.dense.weight','output.dense.weight','attention.self.query.weight','attention.self.key.weight','attention.self.value.weight']:
            target_group = {k:v for k,v in gradient_name_mask.items() if target_name in k}
    ...
    def train(
        ...
        **kwargs,
        ...
        if self.mode == 'ChildTuning-D':
            gradient_mask, gradient_name_mask = self.calculate_fisher()
            choose_name = self.determine_weight_across_layers(gradient_name_mask, topN = self.args.topN)
            for name, params in model.named_parameters():
                if name in choose_name:
                    gradient_mask[params] = params.new_ones(params.size())
                else:
                    gradient_mask[params] = params.new_zeros(params.size())
            print("Check mask ratio: {}M".format(sum([torch.sum(v) for k,v in gradient_mask.items()])/sum([v.numel() for k,v in gradient_mask.items()])))

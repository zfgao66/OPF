...
sys.path.append('/home/zfgao/work/OPF/')
...
def main():
    ...
    training_args.output_dir= custom_config['output_dir']
    ...
    custom_config = custom_args.__dict__
    data_args.eval_tasks = custom_config['eval_tasks'].split(',')
    data_args.tasks = custom_config['tasks'].split(',')
    ...
    # model_ori = T5ForConditionalGeneration.from_pretrained(
    #     model_path,
    #     from_tf=".ckpt" in model_args.model_name_or_path,
    #     config=config,
    #     cache_dir=model_args.cache_dir,
    ...
    if 'word_embed' in config.mpo_layers:
        ...
        del model.shared
        del model.lm_head
    if ('mlp' in config.mpo_layers and 'mlp' not in config.load_layer) or ('attention' in config.mpo_layers and 'attention' not in config.load_layer):
        for i in range(config.num_layers):
            model.encoder.block[i].from_pretrained_mpo()
            model.decoder.block[i].from_pretrained_mpo()
            del model.encoder.block[i].layer[1].DenseReluDense.wi
            del model.encoder.block[i].layer[1].DenseReluDense.wo
            del model.decoder.block[i].layer[2].DenseReluDense.wi
            del model.decoder.block[i].layer[2].DenseReluDense.wo
        ...
        logger.info("Check using moe without mpo")
        for i in range(config.num_layers):
        ...
        logger.info("Check using switch without mpo")
        for i in range(config.num_layers):
    ...
    if training_args.do_eval or training_args.do_test:
        if trainer.is_world_process_zero():
            ...
            if not training_args.do_train:
                ...
                model.load_state_dict(torch.load(os.path.join(last_checkpoint_path,"pytorch_model.bin")))
    ...
    end_time = time.time()
    logger.info("Check eval time init: {}/n after init: {}".format(int(end_time) - int(st_time_init), int(end_time) - int(st_time_after_init)))

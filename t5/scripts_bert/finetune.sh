...
function run_task_ddp() {
    ...
    nohup python -m torch.distributed.launch --nproc_per_node=`get_gpu_count ${1}` --master_addr=`get_init_method` --master_port=$2 finetune_t5_trainer.py configs/finetune.json \

...
import sys
sys.path.append("/home/pyliu/gpt2")
...
class PreTrainedModel(nn.Module, ModuleUtilsMixin, GenerationMixin):
    ...
    def _tie_or_clone_weights(self, output_embeddings, input_embeddings):
        ...
        这里就是完全一样的，在python中对象的赋值直接是绑定的
        """
        if self.config.torchscript:
            # torchscript 暂时还不知道具体是啥，这里暂以此代替看看有什么情况
            output_embeddings.weight = nn.Parameter(input_embeddings.weight.clone())
            raise NotImplementedError('Check torch script not implemented')
        else:
            if getattr(input_embeddings,'weight', None) is None:
                # mpo = MPO(input_embeddings.mpo_input_shape, input_embeddings.mpo_output_shape, input_embeddings.truncate_num)
                # output_embeddings.weight.data = mpo.mpo2matrix(input_embeddings.tensor_set)[:50257]
                output_embeddings.tensor_set[0] = input_embeddings.tensor_set[0]
                output_embeddings.tensor_set[1] = input_embeddings.tensor_set[1]
                output_embeddings.tensor_set[2] = input_embeddings.tensor_set[2]
                output_embeddings.tensor_set[3] = input_embeddings.tensor_set[3]
                output_embeddings.tensor_set[4] = input_embeddings.tensor_set[4]
            else:
...
class Conv1D(nn.Module):
    ...
    def __init__(self, nf, nx, train_AT_index=None):
        super().__init__()
        self.nf = nf
        w = torch.empty(nx, nf)
        ...
        self.train_AT_index = train_AT_index
...
class Conv1D_mpo(nn.Module):
    def __init__(self, mpo_input_shape, mpo_output_shape, trunc_num, tensor_learn=False, train_AT_index=None):
        super().__init__()
        self.tensor_set = None
        self.tensor_learn = tensor_learn
        self.mpo_input_shape, self.mpo_output_shape, self.trunc_num = mpo_input_shape, mpo_output_shape, trunc_num

        nf = torch.prod(torch.tensor(mpo_output_shape))
        self.bias = nn.Parameter(torch.zeros(nf))
        self.train_AT_index = train_AT_index
    ...
    def forward(self, x):
        ...
        # res = torch.addmm(res, res, mpo.mpo2matrix(self.tensor_set)) + self.bias
        res = torch.addmm(self.bias, x.view(-1, x.size(-1)), mpo.mpo2matrix(self.tensor_set))
        ori_shape=x.shape

        return res.view((tuple(ori_shape[:-1])+(-1,)))
    def from_pretrained(self, tensor_set,bias=None,use_bias=True):
        ...
        if self.tensor_learn:
            logger.info('Check requires_grad = false in {}'.format(self.train_AT_index))
            for ind in self.train_AT_index:
                self.tensor_set[ind].requires_grad = False
        if use_bias:
            self.bias = bias
        else:

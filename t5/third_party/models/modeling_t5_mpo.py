...
from transformers.models.t5.modeling_t5 import (T5LayerNorm,load_tf_weights_in_t5, T5Model, T5DenseGatedGeluDense)
#, T5Block, T5Model, T5ForConditionalGeneration,
#                                       T5DenseReluDense, T5Attention, T5LayerCrossAttention,load_tf_weights_in_t5, T5DenseGatedGeluDense)
from transformers.file_utils import DUMMY_INPUTS, DUMMY_MASK
from MPOE.third_party.compress_tools.modeling_t5_utils import (T5Block,EmbeddingMPO,LinearDecomHead, T5DenseReluDense, 
                                                                    T5Attention)
from MPOE.third_party.compress_tools.modeling_utils import PreTrainedModel
from mpo_lab.Matrix2MPO import MPO
import numpy as np
from transformers.utils import logging

from MPOE.adapters import (MetaAdapterConfig,
...
class T5ForConditionalGeneration(T5PreTrainedModel):
    ...
    def __init__(self, config, adapter_config=None):
        ...
        self.use_mpo = False
        if 'word_embed' in config.mpo_layers or 'mlp' in config.mpo_layers or 'attention' in config.mpo_layers:
            self.use_mpo = True
        if 'word_embed' in config.mpo_layers:
            logger.info("Check using EmbeddingMPO")
            # self.mpo_input_shape, self.mpo_output_shape, self.truncate_num = [8,8,8,8,8], [3,4,4,4,4], config.emb_trunc # t5-base
            self.mpo_input_shape, self.mpo_output_shape, self.truncate_num = [8,8,8,8,8], [4,4,4,4,4], config.emb_trunc # t5-large
            self.shared_mpo = EmbeddingMPO(config.vocab_size, config.d_model, self.mpo_input_shape, self.mpo_output_shape, self.truncate_num)
            self.mpo = MPO(self.mpo_input_shape, self.mpo_output_shape, self.truncate_num)
            pad_num = np.prod(self.mpo_input_shape) - config.vocab_size
            self.zeros_dim = (pad_num,) + tuple(self.shared.weight.data.shape[1:])
            self.from_pretrained_mpo()
        else:
            logger.info("Check using original embedding")
        ...
        if self.use_mpo:
            if 'word_embed' in config.mpo_layers:
                self.encoder = T5Stack(encoder_config, self.shared_mpo, adapter_config=adapter_config)
            else:
        ...
        if self.use_mpo:
            if 'word_embed' in config.mpo_layers:
                self.decoder = T5Stack(decoder_config, self.shared_mpo, adapter_config=adapter_config)
            else:
        ...
        if 'word_embed' in config.mpo_layers:
            '''
            为了把Embedding权重完全移动到lm_head又不转置，必须用linear的结构；
            这里只继承LinearDecom类，又得复写forward，因为拿过来的矩阵不能全用，而是应该和EmbeddingMPO一样只取前vocab_size个权重
            '''
            self.lm_head_mpo = LinearDecomHead(self.mpo_input_shape, self.mpo_output_shape, self.truncate_num, bias=False,tensor_learn=self.config.tensor_learn)
            self.lm_head_mpo.mpo = MPO(self.mpo_input_shape, self.mpo_output_shape, self.truncate_num)
            self.lm_head_mpo.tensor_set = self.shared_mpo.tensor_set            
        self.init_weights()

    def get_input_embeddings(self):
        if 'word_embed' in self.config.mpo_layers:
            return self.shared_mpo
        return self.shared
    ...
    def forward(
        ...
        if 'word_embed' in self.config.mpo_layers:
            lm_logits = self.lm_head_mpo(sequence_output)
        else:
    ...
    def from_pretrained_mpo(self):
        if 'word_embed' in self.config.mpo_layers:
            mpo_tensor_set,_,_ = self.mpo.matrix2mpo(
                np.concatenate((self.shared.weight.data.cpu().numpy(), np.zeros(self.zeros_dim).astype('float32')), 0))
            self.shared_mpo.tensor_set = torch.nn.ParameterList(
            ...
            if self.config.tensor_learn:
                ...
                self.shared_mpo.tensor_set[2].requires_grad = False
            else:
                logger.info("Check without tensor_learn")

            if hasattr(self, 'lm_head'):
                mpo_tensor_set, _, _ = self.mpo.matrix2mpo(np.concatenate((self.lm_head.weight.data.cpu().numpy(), np.zeros(self.zeros_dim).astype('float32')), 0))
                self.lm_head_mpo.from_pretrained(None, None, mpo_tensor_set, self.lm_head.bias)